// VORTEX - Fond Spatial Immersif et Sombre
...
class BlackHoleUniverse {
    ...
    initStars() {
        this.stars = [];
        // Milliers d'étoiles très petites
        const starCount = Math.floor((this.canvas.width * this.canvas.height) / 150);
        
        for (let i = 0; i < starCount; i++) {
            const depth = Math.random();
            const isBright = Math.random() < 0.02; // 2% d'étoiles plus lumineuses
            
            this.stars.push({
                x: Math.random() * this.canvas.width,
                y: Math.random() * this.canvas.height,
                size: isBright ? (0.4 + Math.random() * 0.3) : (0.05 + Math.random() * 0.15),
                opacity: isBright ? (0.15 + Math.random() * 0.1) : (0.008 + Math.random() * 0.012),
                depth: depth,
                twinkle: Math.random() * Math.PI * 2,
                twinkleSpeed: (0.003 + Math.random() * 0.005) * (1 - depth * 0.4),
                color: isBright ? '#e8f0ff' : this.getStarColor(depth)
            });
        }
    }
    ...
    initNebulaClouds() {
        ...
        for (let i = 0; i < cloudCount; i++) {
            ...
            this.nebulaClouds.push({
                ...
                opacity: Math.random() * 0.002 + 0.001,
    ...
    initInnerPlasma() {
        ...
        for (let i = 0; i < particleCount; i++) {
            ...
            this.innerPlasma.push({
                angle: angle,
                distance: distance,
                speed: (0.0006 + Math.random() * 0.001) * (100 / distance),
    ...
    initOuterSpiral() {
        ...
        for (let i = 0; i < armCount; i++) {
            ...
            for (let j = 0; j < particleCount; j++) {
                ...
                const radius = 100 + t * 250 + Math.sin(t * Math.PI * 5) * 20;
                particles.push({
                    baseAngle: baseAngle,
                    t: t,
                    angle: angle,
    ...
    getSpiralColor(t) {
        if (t < 0.25) return { r: 8, g: 145, b: 178 };
        if (t < 0.45) return { r: 20, g: 184, b: 166 };
        if (t < 0.65) return { r: 34, g: 197, b: 94 };
        if (t < 0.8) return { r: 245, g: 158, b: 11 };
        return { r: 74, g: 26, b: 107 };
    }

    initCosmicMatter() {
        this.cosmicMatter = [];
        const matterCount = 500;
        ...
        for (let i = 0; i < matterCount; i++) {
            ...
            this.cosmicMatter.push({
                angle: angle,
                distance: distance,
                speed: (0.0002 + Math.random() * 0.0004) * (550 / distance),
    ...
    drawNebulaClouds() {
        this.nebulaClouds.forEach(cloud => {
            ...
            const pulse = 1 + Math.sin(cloud.pulsePhase) * 0.08;
            
            const gradient = this.ctx.createRadialGradient(
                cloud.x, cloud.y, 0,
                cloud.x, cloud.y, cloud.radius * pulse
            );
            gradient.addColorStop(0, cloud.color.replace('0.04', (cloud.opacity * 1.5).toString()).replace('0.03', (cloud.opacity * 1.3).toString()).replace('0.025', (cloud.opacity * 1.2).toString()).replace('0.02', (cloud.opacity * 1.1).toString()));
            gradient.addColorStop(0.5, cloud.color.replace('0.04', (cloud.opacity * 0.6).toString()).replace('0.03', (cloud.opacity * 0.5).toString()).replace('0.025', (cloud.opacity * 0.45).toString()).replace('0.02', (cloud.opacity * 0.4).toString()));
    ...
    drawShootingStars() {
        ...
        this.shootingStars = this.shootingStars.filter(star => {
            ...
            gradient.addColorStop(0, `rgba(232, 240, 255, ${star.opacity * star.life})`);
            gradient.addColorStop(1, 'transparent');
    ...
    drawOuterSpiral() {
        this.outerSpiral.forEach(arm => {
            arm.forEach(particle => {
                ...
                particle.angle = particle.baseAngle + particle.t * Math.PI * 1.5 + this.angle * 0.35;
    ...
    drawHaloLayers() {
        this.haloLayers.forEach(halo => {
            ...
            const pulse = 1 + Math.sin(halo.pulsePhase) * 0.08;
            ...
            gradient.addColorStop(0.5, halo.color.replace('0.08', (parseFloat(halo.color.match(/[\d.]+/)[0]) * 0.5).toString()).replace('0.06', (parseFloat(halo.color.match(/[\d.]+/)[0]) * 0.45).toString()).replace('0.05', (parseFloat(halo.color.match(/[\d.]+/)[0]) * 0.4).toString()).replace('0.04', (parseFloat(halo.color.match(/[\d.]+/)[0]) * 0.35).toString()));
            gradient.addColorStop(1, 'transparent');
    ...
    drawBlackHole() {
        ...
        distortionHalo.addColorStop(0, 'rgba(74, 26, 107, 0.008)');
        distortionHalo.addColorStop(0.5, 'rgba(30, 58, 138, 0.006)');
