...
global.SENDER_EMAIL_NOTIF_TEST = 'PetitsDjobs <onboarding@resend.dev>'; // Défini globalement pour éviter les ReferenceError
...
app.get('/api/mes-missions-prestataire', requireAuth, async (req, res) => {
    ...
    console.log(`✅ [NOTIF-STEP-2] ${missions.length} mission(s) brute(s) trouvée(s) pour ${pId}`);
...
app.post('/api/marquer-mission-vue/:id', requireAuth, async (req, res) => {
    ...
    // On nettoie aussi la session pour arrêter les alertes immédiates
    if (req.session.commande && req.session.commande.missionId == mId) {
        req.session.commande.vu_par_prestataire = true;
    }
    res.json({ ok: true });
});
...
app.post('/api/repondre-mission', requireAuth, async (req, res) => {
    ...
    const mId = parseInt(missionId, 10); const pId = req.session.user.id;
    ...
    let payload = { statut: action === 'accepter' ? 'en_route' : 'refuse' };
    if (action === 'accepter' && estSecours) {
        payload.prestataire_id = pId;
        payload.backup_ids = (mission.backup_ids || []).filter(id => id !== pId);
    }

    const { error } = await supabase.from('missions').update(payload).eq('id', mId);
    if (error) return res.status(500).json({ error: error.message });
    res.json({ ok: true });
});
...
app.post('/api/confirmer-fin-travail', requireAuth, async (req, res) => {
    ...
    // Mettre à jour le statut et le flag client_a_confirme_fin dans la DB
...
app.post('/annuler-job/:id', requireAuth, async (req, res) => {
    ...
    // Suppression de l'envoi d'emails aux prestataires
